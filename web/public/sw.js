...
self.addEventListener('fetch', (event) => {
  ...
  // /farm/*, /mahjong/*, /blockpuzzle/*, /gunny/*, /garden/* and /worms/* are sibling games served by their own
  // services behind the same host (each with its own service worker) — let them hit
  // the network untouched (and never overwrite the cached chat shell).
  const { pathname } = new URL(event.request.url);
  if (['/farm', '/mahjong', '/blockpuzzle', '/gunny', '/garden', '/worms'].some((prefix) => pathname.startsWith(prefix))) return;
