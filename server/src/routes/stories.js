...
export async function registerStoryRoutes(app) {
  ...
  app.get('/stories', { preHandler: requireUser }, async (request, reply) => {
    ...
    const rows = db
      .prepare(
        `SELECT s.id, s.user_id, s.kind, s.created_at, u.username, u.display_name, u.avatar_at
         FROM stories s
    ...
    for (const row of rows) {
      if (!groups.has(row.user_id)) {
        groups.set(row.user_id, { user: { id: row.user_id, username: row.username, display_name: row.display_name || row.username, avatar_at: row.avatar_at || null }, stories: [] });
      }
